...
class ReplayBuffer(BaseBuffer):
    ...
    def add(self, obs: np.ndarray, next_obs: np.ndarray, action: np.ndarray, reward: np.ndarray, value: th.Tensor, done: np.ndarray) -> None:
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        if self.full:
            ...
            #batch_inds = np.array([(self.pos + i) % self.buffer_size for i in range(1, batch_size + 1)])
        else:
            batch_inds = np.random.randint(0, self.pos, size=batch_size)
            #batch_inds = np.array([i % (self.pos + 1) for i in range(batch_size)])
        return self._get_samples(batch_inds, env=env)
